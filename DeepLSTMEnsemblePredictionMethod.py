...
class DeepLSTMEnsemblePredictionMethod(NeuralNetworkPredictionMethod):
    ...
    def constructModel(self, period):
        ...
        if self.numNeurons[0] == None: 
            for q in range(0, self.numTestPoints):
               ...
               model = Sequential()
               model.add(LSTM(lstm1NeuronCount, input_shape=(self.window,1), activation='tanh', return_sequences=True))
               ...
               self.layer_description={}
               self.layer_description['model' + str(period) + 'layer1_type']="LSTM"
               ...
               self.layer_description['model' + str(period) + 'layer2_type']='Dropout'
               self.layer_description['model' + str(period) + 'layer2_dropout']='0.1'
               self.layer_description['model' + str(period) + 'layer3_type']='LSTM'
               self.layer_description['model' + str(period) + 'layer3_neuron_count']=3
               self.layer_description['model' + str(period) + 'layer3_activation']='tanh'
               self.layer_description['model' + str(period) + 'layer4_type']='Dropout'
               self.layer_description['model' + str(period) + 'layer4_dropout']=0.1
               self.layer_description['model' + str(period) + 'layer5_type']="Dense"
               self.layer_description['model' + str(period) + 'layer5_neuron_count']=self.numTestPoints
               self.layer_description['model' + str(period) + 'layer5_activation']="tanh"
